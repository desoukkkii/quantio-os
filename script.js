(() => {
  ...
  const WALLPAPERS = {
    sunset: {
      label: "Sunset",
      colors:
        "linear-gradient(135deg, #0f1115 0%, #1a1d2e 40%, #2a1e2e 70%, #0f1115 100%)",
    },
    ocean: {
      label: "Ocean",
      colors:
        "linear-gradient(135deg, #0a1628 0%, #0d2847 40%, #163a5c 70%, #0a1628 100%)",
    },
    forest: {
      label: "Forest",
      colors:
        "linear-gradient(135deg, #0a1a0f 0%, #142818 40%, #1a3a22 70%, #0a1a0f 100%)",
    },
    space: {
      label: "Space",
      colors:
        "linear-gradient(135deg, #050510 0%, #100820 40%, #1a0a2e 70%, #050510 100%)",
    },
    light: {
      label: "Light",
      colors:
        "linear-gradient(135deg, #d0d0d8 0%, #e8e8f0 40%, #f0f0f8 70%, #d0d0d8 100%)",
    },
  };
  const ACCENT_COLORS = [
    "#0A84FF",
  ...
  const TERMINAL_COMMANDS = {
    ...
    echo: {
      desc: "Echo text",
      fn: (args, term) => term.writeln(args.join(" ") || ""),
    },
    date: {
      desc: "Show current date and time",
      fn: (args, term) => term.writeln(new Date().toString()),
    },
    whoami: {
      desc: "Show current user",
      fn: (args, term) => term.writeln("quantio"),
    },
    ...
    ls: {
      ...
      fn: (args, term) =>
        term.writeln(
          "Applications  Desktop  Documents  Downloads  Music  Pictures  Movies",
        ),
    },
    pwd: {
      desc: "Print working directory",
      fn: (args, term) => term.writeln("/Users/quantio"),
    },
    neofetch: {
        ...
        term.writeln(
          "  Browser: " +
            navigator.userAgent.match(
              /(Chrome|Firefox|Safari|Edge)\/?\s*(\d+)/,
            )?.[0] || "Unknown",
        );
        term.writeln(
    ...
    calc: {
      ...
      fn: (args, term) => {
        ...
        term.writeln("Opening Calculator...");
      },
    },
    notes: {
      desc: "Open notes",
      fn: (args, term) => {
        OS.openApp("notes");
        term.writeln("Opening Notes...");
      },
    },
    uptime: {
      desc: "Show system uptime",
      fn: (args, term) => {
        if (!OS.state.bootTime) {
          term.writeln("System just started");
          return;
        }
    ...
    theme: {
      ...
      fn: (args, term) => {
        ...
        if (hex && /^#[0-9a-f]{6}$/i.test(hex)) {
          ...
          term.writeln(
            "Unknown color. Try: blue, red, green, yellow, purple, orange, cyan",
          );
        }
      },
    },
    wall: {
      ...
      fn: (args, term) => {
        ...
        if (WALLPAPERS[name]) {
          ...
          term.writeln(
            "Unknown wallpaper. Try: sunset, ocean, forest, space, light",
          );
        }
      },
    },
  ...
  class Desktop {
    ...
    renderIcons() {
      if (!OS.state.settings.showDesktopIcons) {
        OS.els.desktopIcons.innerHTML = "";
        return;
      }
      OS.els.desktopIcons.innerHTML = DESKTOP_ICONS.map(
        (ic) =>
    ...
    setupClickDismiss() {
      document.addEventListener("click", (e) => {
        ...
        if (
          !e.target.closest("#spotlight") &&
          !e.target.closest('.menubar-item[data-action="spotlight"]')
        ) {
      ...
      document.querySelectorAll(".apple-item").forEach((el) => {
        el.addEventListener("click", () => {
          ...
          if (action === "about")
            OS.notify(
              "Quantio OS",
              "Version 1.0 \u00B7 Built with web technologies",
            );
          if (action === "sleep") {
            OS.classes.lock.show();
          }
          ...
          if (action === "shutdown") {
            document.body.innerHTML =
              '<div style="display:flex;align-items:center;justify-content:center;height:100vh;background:#000;color:#fff;font-family:system-ui;font-size:18px;flex-direction:column;gap:16px"><span>\u23F9\uFE0F</span><span>Shut Down</span></div>';
          }
        });
      });
    }
      ...
      container.querySelectorAll(".dock-item").forEach((el) => {
        ...
        el.addEventListener("click", () => {
          ...
          if (existing.length > 0) {
            ...
            if (firstClosed) {
              wm.restoreWindow(firstClosed.id);
            } else if (firstOpen) {
              wm.focus(firstOpen.id);
            } else {
              wm.focus(existing[0].id);
            }
          } else {
            OS.openApp(id);
          }
        ...
        el.addEventListener("contextmenu", (e) => {
          ...
          if (OS.state.runningApps.has(id)) {
            items.push({
              ...
              action: () => {
                existing.forEach((w) => {
                  if (!w.minimized) OS.classes.wm.minimize(w.id);
                });
              },
            });
            items.push({
  ...
  class WindowManager {
    ...
    focus(id) {
      ...
      if (el) {
        ...
        document.querySelectorAll(".window.focused").forEach((e) => {
          if (e.id !== id) e.classList.remove("focused");
        });
      }
    ...
    afterClose(win) {
      if (this.windows.size === 0) {
        ...
        const last = Array.from(this.windows.values())
        ...
        if (last) this.focus(last.id);
          ...
          const any = Array.from(this.windows.values()).find(
            (w) => !w.minimized,
          );
    ...
    setupDrag(el, win) {
      ...
      tb.addEventListener("mousedown", (e) => {
        ...
        document.addEventListener(
          "mousemove",
          (this._dragMove = (e) => this.onDrag(e)),
        );
        document.addEventListener(
          "mouseup",
          (this._dragEnd = () => this.endDrag()),
        );
        e.preventDefault();
      });
    }
    ...
    startResize(e, id, dir) {
      ...
      document.addEventListener(
        "mousemove",
        (this._resizeMove = (e) => this.onResize(e)),
      );
      document.addEventListener(
        "mouseup",
        (this._resizeEnd = () => this.endResize()),
      );
    }

    onResize(e) {
      if (!this._resizeState) return;
      const { win, dir, startX, startY, origX, origY, origW, origH } =
        this._resizeState;
      const dx = e.clientX - startX;
      const dy = e.clientY - startY;
      let x = origX,
    ...
    buildContent(appId, el) {
      ...
      if (builders[appId]) builders[appId]();
        ...
        el.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-tertiary);font-size:13px">${appId} app</div>`;
    }
    ...
    buildFinder(el) {
      ...
      sidebar.innerHTML = SIDEBAR_ITEMS.map(
        (it) =>
      ...
      sidebar.querySelectorAll(".finder-nav").forEach((n) => {
        n.addEventListener("click", () => {
          sidebar
            .querySelectorAll(".finder-nav")
            .forEach((x) => x.classList.remove("active"));
          n.classList.add("active");
        });
      });
      const filesEl = el.querySelector("#finder-files");
      filesEl.innerHTML = FINDER_FILES.map(
        (f) =>
    ...
    buildCalculator(el) {
      ...
      const update = () => {
        resultEl.textContent =
          state.current.length > 14
            ? parseFloat(state.current).toExponential(4)
            : state.current;
        exprEl.textContent = state.operator
      ...
      const calculate = () => {
        ...
        state.current =
          typeof r === "number" ? String(parseFloat(r.toFixed(10))) : "Error";
        state.prev = "";
        state.operator = null;
        state.reset = true;
        update();
    ...
    _loadNotes(el) {
        ...
        notes = [
          {
            ...
            title: "Welcome",
            content:
              "Welcome to Quantio Notes!\n\nYour notes are automatically saved.",
          },
          {
            id: "note-2",
            title: "Getting Started",
            content:
              "Type in the editor to create notes.\nClick + to add a new note.",
          },
        ];
      ...
      const renderEditor = () => {
        ...
        if (!note) {
          editorEl.innerHTML = emptyEl.outerHTML;
          return;
        }
        editorEl.innerHTML = `
      ...
      const addBtn =
        ...
        (() => {
          ...
          const tb =
            el.querySelector(".app-toolbar") ||
            el.querySelector(".notes")?.querySelector(".app-toolbar");
    ...
    buildPhotos(el) {
      ...
      const render = () => {
        ...
        el.innerHTML = `<div class="photos">
          ...
          <div class="photos-strip" id="ph-strip">${PHOTOS_SVGS.map(
            (p, i) =>
              `<div class="photos-thumb ${i === currentIdx ? "active" : ""}" data-idx="${i}">${p.svg}</div>`,
          ).join("")}</div>
        </div>`;
        el.querySelector("#ph-prev")?.addEventListener("click", () => {
          if (currentIdx > 0) {
            currentIdx--;
            render();
          }
        });
        el.querySelector("#ph-next")?.addEventListener("click", () => {
          if (currentIdx < PHOTOS_SVGS.length - 1) {
            currentIdx++;
            render();
          }
        });
    ...
    buildSettings(el) {
      ...
      const render = () => {
        ...
        this._renderSettingsPage(
          el.querySelector("#settings-content"),
          activePage,
        );
      };
      render();
    ...
    _renderSettingsPage(el, page) {
      ...
      if (page === "general") {
        ...
        el.querySelector("#toggle-icons")?.addEventListener(
          "click",
          function () {
            OS.updateSettings({
              showDesktopIcons: !OS.state.settings.showDesktopIcons,
            });
        ...
        grid.innerHTML = Object.entries(WALLPAPERS)
          .map(
            ([k, v]) =>
        ...
        grid.innerHTML = ACCENT_COLORS.map(
          (c) =>
        ...
        range.addEventListener("input", () => {
          ...
          document
            ...
            .forEach((el) => (el.style.fontSize = 24 * v + "px"));
  ...
  class Spotlight {
    ...
    search(q) {
      const results = DOCK_APPS.filter(
        (a) =>
          a.label.toLowerCase().includes(q.toLowerCase()) ||
          a.desc.toLowerCase().includes(q.toLowerCase()),
      );
      if (q === "") {
        OS.els.spotlightResults.innerHTML =
          '<div style="padding:12px 16px;font-size:11px;color:var(--text-tertiary)">Type to search apps...</div>';
        return;
      }
      ...
      OS.els.spotlightResults
        .querySelectorAll(".spotlight-result")
        .forEach((el) => {
          el.addEventListener("click", () => {
    ...
    setup() {
      OS.els.spotlightInput.addEventListener("input", () =>
        this.search(OS.els.spotlightInput.value),
      );
      OS.els.spotlightInput.addEventListener("keydown", (e) => {
        if (e.key === "Escape") this.close();
        if (e.key === "Enter") {
          const first =
            OS.els.spotlightResults.querySelector(".spotlight-result");
          if (first) {
            OS.openApp(first.dataset.app);
            this.close();
          }
        }
      });
        ...
        items = [
          {
            label: "New Folder",
            icon: "\uD83D\uDCC1",
            action: () => OS.notify("Desktop", "New folder created"),
          },
          {
            label: "Get Info",
            icon: "\u2139\uFE0F",
            action: () => OS.notify("Info", "Quantio OS Desktop"),
          },
          { type: "sep" },
          {
            label: "Change Wallpaper",
            icon: "\uD83D\uDDBC",
            action: () => OS.openApp("settings"),
          },
          {
            label: "Open Settings",
            icon: "\u2699\uFE0F",
          ...
          { type: "sep" },
          {
            ...
            icon: "\uD83D\uDCBB",
            checked: OS.state.settings.showDesktopIcons,
            action: () =>
              OS.updateSettings({
                showDesktopIcons: !OS.state.settings.showDesktopIcons,
              }),
  ...
  class NotificationManager {
    add(title, msg, icon) {
      ...
      n.querySelector(".notif-close").addEventListener("click", (e) => {
        e.stopPropagation();
        this.remove(n);
      });
